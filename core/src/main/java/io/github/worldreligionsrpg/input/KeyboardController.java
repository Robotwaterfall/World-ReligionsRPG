...
public class KeyboardController extends InputAdapter {
    private static final Map<Integer, Command> KEY_MAPPING = Map.ofEntries(
            Map.entry(Input.Keys.W, Command.UP), Map.entry(Input.Keys.S, Command.DOWN),
            Map.entry(Input.Keys.A, Command.LEFT), Map.entry(Input.Keys.D, Command.RIGHT),
    ...
    public void setActiveState(Class<? extends ControllerState> stateClass) {
        ...
        if (state == null) {
            throw new GdxRuntimeException(
                    "State " + stateClass.getSimpleName() + " not found in cache");
        }
